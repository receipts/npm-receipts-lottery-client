...
var buildForm = function buildForm(authRequest, token) {
  logger.args('buildForm', arguments);
...
var authorizeUser = function authorizeUser(authRequest, options, callback) {
  ...
  return clientMain.getMainPageData(jar, options, function (error, data) {
    ...
    var form = buildForm(authRequest, data.token);

    return auth(jar, form, data.token, authFormConfig, function (err, data) {
      if (err) {
