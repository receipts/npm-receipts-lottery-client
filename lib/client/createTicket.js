...
var buildForm = function buildForm(ticketRequest, captcha) {
  ...
  var trade = tradeConverter.getIdFromTrade(ticketRequest.trade);
...
var sendLotteryTicket = function sendLotteryTicket(ticketRequest, options, callback) {
  ...
  return mainClient.getMainPageData(jar, options, function (err, data) {
    ...
    var createTicketsRequestConfig = options.get('create-ticket-request');
    assert.object(createTicketsRequestConfig, 'createTicketsRequestConfig');

    if (data.url) {
      createTicketsRequestConfig.url = data.url;
    }
